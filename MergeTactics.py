...
def Detector2(img_name, elixir_value):
    global Elixir
    while True:
        ...
        try:
            res = pyautogui.locateOnScreen(
                os.path.join(elixir_folder_dir, img_name),
                confidence=0.9,
                grayscale=True,
...
def play_card(card_name, location, elixir_cost):
    ...
    print(f"[BOT] Played {card_name}, Remaining Elixir: {bot_memory['elixir']}")
...
def bot_brain_loop():
    while True:
        ...
        # Play first available priority card
        for card_img, center, cost in priority_playables:
            ...
            if current_elixir >= cost:
                ...
                time.sleep(0.5)
                break  # Play only one priority card per cycle

        else:
            # If no priority card was played, play a regular one
...
for i in range(11):
    img = f"{i}.png"
    thread = threading.Thread(target=Detector2, args=(img, i))
    thread.start()
